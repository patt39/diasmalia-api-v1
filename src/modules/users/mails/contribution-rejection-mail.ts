...
export const contributorRejectionMail = async (options: {
  contributor: any;
  email: string;
}) => {
  const { email, contributor } = { ...options };
  const output = `
  ...
  <html xmlns="http://www.w3.org/1999/xhtml">
    ...
    >
                    ...
                    <tr>
                      ...
                      <div style="text-align: center">
                        <span style="font-size: 16px">
                            ${contributor?.user.profile.firstName} ${contributor?.user.profile.lastName} with email ${email}has rejected your invitation to join your organization
                             </span
                          >
                      </div><br />
      ...
      `;
  await sendEmail({
    from: `${contributor?.user?.email}`,
    to: [email],
    subject: `Invitation rejection}`,
    html: output,
  });
};
