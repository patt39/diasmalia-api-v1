...
export const reportPDFAttachment = async (options: {
  email: string;
  animal: any;
  filename: string;
  content: any;
}) => {
  const { email, filename, content, animal } = options;
  const output = `
                    ...
                    <tr>
                      ...
                      <div style="text-align: center">
                        <span style="font-size: 16px">
                           Rapport de fin de production de la bande ${animal.code} de ${animal.animalType.name} en piece jointe </span
                          >
                      </div><br />
  ...
  await sendEmail({
    ...
    to: [email],
    subject: `${config.datasite.name} - Production report pdf attachment`,
    html: output,
